...
const aiFilter = async (candidates) => {
  ...
  const prompt = `你是個殺伐果斷但說話接地氣的台股老手。請閱讀以下股票的最新新聞，判斷這檔股票有沒有『實質業績利多』（如：公司真的賺大錢、接到大客戶訂單）。
評分標準：
- 1~3 星：只是在炒新聞、講故事，或是新聞根本沒重點。
- 4~5 星：公司真的拿出賺錢的鐵證，有實質的爆發點。
請以 JSON 格式回傳陣列，對【所有】傳入的股票進行評分（1~5星），並給出「一句話的理由」。
⚠️ 語氣與理由要求：
1. 必須解釋「為什麼」你給這個星數（背後的原因）。
2. 【絕對不要用任何專業財經術語】，把話說到連菜市場阿嬤、不懂股票的大學生都能一聽就懂！
3. 極度白話、直接、帶點江湖味！不要打官腔，絕對不要說「建議觀望」這種廢話！直接點破這是在玩真的還是假動作！
格式範例：[{"symbol": "2330", "stars": 5, "reason": "這家公司上個月多賺了快一半，絕對是抱到超級大客戶的大腿了，這業績是玩真的，準備要噴了！"}]
